...
{
    $j = 0;
    /* Cette partie supprime les liens ayant été mis plusieurs fois et les liens vides */
    $uniqueValues = array();
    foreach ($urls as $key => $value) {
        // Check if the value is a duplicate or an empty string
        if (empty($value) || in_array($value, $uniqueValues)) {
            // If the value is a duplicate or empty, remove it from the arrays
            unset($urls[$key]);
            unset($formats[$key]);
            ...
            // If the value is unique and not empty, add it to the $uniqueValues array
            $uniqueValues[] = $value;
        }
        if ($selectedElements[$key] != -1) {
            // If we're on a link with a timer, move onto the next timer
            $j = $j + 2;
        }
    }
    // Re-index the arrays after removing duplicates and empty values
